...
int read_from_binary_file()
{
	...
	while(fread(&access, sizeof( access ) , 1 , FILE_HANDLE) == 1)
	{
		fprintf(stdout,"\r\nStudent Name:%s\r\nStudent Number:%d\r\nStudent Record:%d"							
	...
	fprintf(stdout,"The file pointer has been reset to the beginning of the file\r\n");
	fprintf(stdout,"Printing list again\r\n");
	...
	if(feof(FILE_HANDLE))
	{
		fprintf(stdout,"\r\nThis is the end of the list again\r\n");
	}
...
int seek_record()
{
	...
	fseek( FILE_HANDLE , ( record_to_search_for - 1)*sizeof(access),SEEK_SET);
