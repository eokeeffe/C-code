...
int update_database()
{
	...
	if(FILE_HANDLE == NULL)
	{
		printf("part_database.txt doesn't exist or cannot be read\r\n");
		exit(0);
	}
	else
	{
...
int menu()
{
	...
	if(user_choice == 4){ sort_database();}
	if(user_choice == 5){ search_database();}
	if(user_choice == 6){ exit_program();}
	else
	{
