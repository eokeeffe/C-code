...
int add_parts()
{
	strcpy( list[0].part_name , "16x16_VGA_display"); // the only way of putting a name into a struct
	strcpy( list[0].part_section , "Phone_Parts");   // is by using strcpy
	list[0].part_number = 1;
	list[0].part_price = 16.50;
	list[0].part_rating = 4.5;
	
	strcpy( list[1].part_name , "AVR_ATMEGA16");
	strcpy( list[1].part_section , "Micro-Controllers");  
