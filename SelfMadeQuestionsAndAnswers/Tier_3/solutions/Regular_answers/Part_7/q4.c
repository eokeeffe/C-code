...
struct shop_parts *swap_node( struct shop_parts *to_swap, struct shop_parts *swap_with );
...
struct shop_parts *swap_node( struct shop_parts *to_swap, struct shop_parts *swap_with )//swap the pointers
{
    struct shop_parts *temp;
	temp = swap_with -> next_struct ;
    swap_with-> next_struct   = to_swap;
    to_swap-> next_struct  = temp;
...
int main()
{
	...
	else
	{
		...
		print_part(next);
		print_part(next->next_struct);//now next points back to the head instead of the tail
